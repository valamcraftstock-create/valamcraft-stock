...
export default function Sales() {
  ...
  const handlePrintReceipt = () => {
      ...
      doc.setFontSize(16); doc.text(profile.storeName || "StockFlow Store", 14, 15);
      doc.setFontSize(9);
      const details = [profile.addressLine1, profile.addressLine2, profile.phone, profile.gstin].filter(Boolean);
      doc.text(details, 14, 22);
      doc.text("Tax Invoice", pageWidth / 2, 45, { align: "center" });
      autoTable(doc, {
          startY: 60,
          head: [['#', 'Item', 'Qty', 'Price', 'Disc', 'Tax', 'Total']],
          body: transactionComplete.items.map((i, idx) => [idx+1, i.name, i.quantity, i.sellPrice, i.discountAmount, transactionComplete.taxLabel || '-', (i.sellPrice * i.quantity - (i.discountAmount || 0)).toFixed(2)]),
          theme: 'striped'
      });
    ...
    <div className={`h-full flex flex-col md:grid md:grid-cols-12 gap-4 pb-0 md:pb-0 ${isReturnMode ? 'bg-orange-50/30' : 'bg-background'}`}>
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[80] flex items-center justify-center p-4">
              <Card className="w-full max-w-md animate-in zoom-in-95 shadow-2xl">
                  ...
                  <CardContent className="space-y-4 pt-4">
                      {checkoutError && <div className="text-destructive text-[11px] bg-destructive/10 p-2 rounded flex items-center gap-2 font-medium"><AlertCircle className="w-3.5 h-3.5" /> {checkoutError}</div>}
                      ...
                      {customerTab === 'search' ? (
                          ...
                          <div className="space-y-3 animate-in fade-in zoom-in-95 duration-200">
                              ...
                              <div className="space-y-1.5">
                                  ...
                                  <Input placeholder="9876543210" value={newCustomerPhone} onChange={e => setNewCustomerPhone(e.target.value)} />
                              </div>
                          </div>
                      )}
      ...
      {transactionComplete && (
          <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[90] flex items-center justify-center p-4">
              <Card className="w-full max-w-sm text-center shadow-2xl animate-in zoom-in">
